...
<script type="text/javascript">
	...
	function order(num, userid){
		...
		location.href = "CartOrderConfirmServlet?num="+num+"&&userid="+userid;
	}
	
	
	function delAllCart(n){
		console.log(n);
		$(n).attr("action", "CartDelAllServelt");
		$(n).submit();
	}
	
	// 수량 증가, 감소
	function amountup(n){
		var num = parseInt($("#cartAmount"+n).attr("value"))+1
		$("#cartAmount"+n).attr("value",num); 
		
		var price = parseInt(($("#price"+n).text()).trim());
		console.log($("#sum"+n).text(price*num));
	}
	function amountdown(n){
		if($("#cartAmount"+n).attr("value") > 1){
			var num = parseInt($("#cartAmount"+n).attr("value"))-1
			$("#cartAmount"+n).attr("value",num);
	...
	$(function(){
		...
		$.ajax({
			...
			data:{
				...
				gAmount: $("#cartAmount"+$(this).attr("data-xxx")).val(),	//이 데이터가 없으면 파라미터로 전달하지 못한다.
				gPrice: $("#price"+$(this).attr("data-xxx")).text()	//이 데이터가 없으면 파라미터로 전달하지 못한다.
			},
			success: function(data, status, xhr){
				$("#sum"+$(self).attr("data-xxx")).text(data);
			},
...
%>
	...
	<tr>
		<td align="center" colspan="5"><a class="a_black"
			...
			<a class="a_black" href="javascript:delAllCart2()"> 전체 삭제하기 </a>&nbsp;&nbsp;&nbsp;&nbsp;
			<a class="a_black" href="javascript:delAllCart(myForm)"> 전체 삭제하기2 </a>&nbsp;&nbsp;&nbsp;&nbsp;
