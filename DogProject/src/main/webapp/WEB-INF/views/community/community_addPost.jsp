<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
<script type=text/javascript charset=utf-8 src="<%=request.getContextPath()%>/resources/ckeditor/ckeditor.js"></script>
<script>
$(function () {
	CKEDITOR.replace('content', {
		filebrowserUploadUrl : '${pageContext.request.contextPath}/adm/fileupload.do'
	});
	
	
	
	$('#postTags').on('keydown', function(e) {
	  if ($(this).val() === '' && e.key !== 'Backspace' && e.key === ' ') {
	    e.preventDefault();
	  } else if ($(this).val() === '' && e.key !== 'Backspace') {
	    $(this).val('#');
	  } else if (e.key === '#') {
	    e.preventDefault();
	    alert('#을 사용할 수 없습니다.');
	  } else if (e.key === ' ') {
	    e.preventDefault();
	    var currentValue = $(this).val();
	    if (currentValue.endsWith(' #')) {
	      return false;
	    }
	    $(this).val($(this).val() + ' #');
	  } else if (e.key === 'Backspace' && $(this).val().endsWith('#')) {
	    e.preventDefault();
	    var currentValue = $(this).val();
	    $(this).val(currentValue.substring(0, currentValue.length - 2));
	  }
	});

});


</script>

</head>
<body>
<jsp:include page = "../common/top.jsp" flush="true"/><br>
<jsp:include page = "../common/side.jsp" flush="true"/><br>
<div style="width: 80%; margin: auto;">
    <form action="addPost" method="post" enctype="multipart/form-data">

        <label for="postTitle">제목 입력:</label><br>
        <input type="text" id="postTitle" name="title" placeholder="제목을 입력하세요." style="width:40%"><br><br>

		<label>첨부 파일:</label><br>
    <input type="file" name="files" multiple/><br><br>
  

        <label for="content" >글 입력:</label><br>
       <textArea id="editor" name="content" style="display:none"></textArea><br>
        
	<label for="postTags">태그 입력:</label><br>
        <input type="text" id="postTags" name="tag"  placeholder="태그를 입력하세요. spacebar로 구분됩니다." style="width:50%"><br><br>
	
        <button type="submit" class="submitButton">등록</button>
      
    </form>
</div>

<br>
</body>
<jsp:include page = "../common/footer.jsp" flush="true"/><br>


</html>